...
func _build_checks() -> void:
	...
	function_displays_text.description = tr("The 'show_text' function should properly display the text")
	...
	function_displays_text.checker = func() -> String:
		...
		for index in _practice.items.size():
			...
			_practice.show_text()
			if _practice.rich_text_label.text != _practice.items[index]:
				_practice.item_index = 0
				return tr("It looks like 'show_text' isn't setting the text properly. We expected the text to be '%s', but it is '%s'"%[_practice.items[index], _practice.rich_text_label.text])
		_practice.item_index = 0
		return ""

	var button_calls_the_function := Check.new()
	button_calls_the_function.description = tr("The 'advance' function should trigger 'show_text' when the button is pressed")
	...
	button_calls_the_function.checker = func() -> String:
		...
		for index in _practice.items.size():
			...
			if _practice.rich_text_label.text != _practice.items[next_index]:
				return tr("It looks like 'show_text' isn't being called when pressing the button. Did you remember to call it?")
		_practice.item_index = 0
		return ""

	checks = [check_index_is_incremented, function_displays_text, button_calls_the_function]
